...
export function CapitalOnboardingModal({
  visible,
  onSaved,
}: {
  visible: boolean;
  ...
  async function save() {
    if (!session || !Number.isFinite(capital) || capital < 0)
      return Alert.alert("Enter a valid starting capital");
    setBusy(true);
      ...
      <View style={styles.overlay}>
        ...
        >
          <View style={[styles.icon, { backgroundColor: palette.primarySoft }]}>
            <Ionicons name="wallet-outline" size={38} color={palette.primary} />
          </View>
          ...
          />
          <Button
            title={busy ? "Saving…" : "Start tracking"}
            onPress={save}
            disabled={
              busy || !Number.isFinite(capital) || capital < 0 || !amount.trim()
            }
          />
